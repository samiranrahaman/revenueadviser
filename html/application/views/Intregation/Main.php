							      ...
							      <div class="alert alert-danger alert-dismissible" ng-show="validationError">
									...
									<div class="modal-body"> 
										...
										<div class="form-group"  ng-class="{ 'has-error' : loginForm.username.$invalid && !loginForm.username.$pristine }">
											<input type="text" class="form-control1" placeholder="Store Name" id="account_name" name="account_name" ng-model="store.account_name" required >
												<p ng-show="loginForm.account_name.$invalid && !loginForm.account_name.$pristine" class="help-block">Store Name is required.</p>
										 </div>
										<div class="form-group"  ng-class="{ 'has-error' : loginForm.username.$invalid && !loginForm.username.$pristine }">
											<input type="text" class="form-control1" placeholder="Apikey" id="account_name" name="account_name" ng-model="store.apikey" required >
										 ...
										 <div class="form-group"  ng-class="{ 'has-error' : loginForm.password.$invalid && !loginForm.password.$pristine }">
											<input type="text" class="form-control1" placeholder="Private app Password" id="password" name="password" ng-model="store.password" required >
										 ...
										 <div class="form-group"  ng-class="{ 'has-error' : loginForm.Url.$invalid && !loginForm.Url.$pristine }">
											<input type="text" class="form-control1" placeholder="Store Url" id="account_name" name="account_name" ng-model="store.url" required >
							      ...
							      <div class="alert alert-danger alert-dismissible" ng-show="validationError">
									...
									<div class="modal-body">
										...
										<div class="form-group"  ng-class="{ 'has-error' : fbloginForm.ads_act.$invalid && !fbloginForm.ads_act.$pristine }">
											<input type="text" class="form-control1" placeholder="FB Ads Account Id" id="ads_act" name="ads_act" ng-model="ads_act" required >
								...
								<div class="col-md-12 graphs">
									...
									<div class="col-md-12">
									 ...
									 <div class="clearfix"> </div>       
										...
										<div class="iinttitile">
											
											CONNECT ACCOUNT WITH REVENUE ADVIZER
											
										</div>
									
									<div class="clearfix"> </div> 
								
										<div class="row topmar shopyf"> 
										<div class="col-sm-4">
										...
										<div class="col-sm-4">
											 <button class="sfbtn wdtbtn" ng-click="showComplex()">Connect Shopify Store</button>
											</div>
										</div>
										...
										<div class="col-sm-4 fbcolr">
											<i class="fa fa-facebook-square fa-2x " ></i>
										</div>
										<div class="col-sm-4 fbcolr">Facebook Account</div>
										<div class="col-sm-4">
										     <button class="fbbtn wdtbtn" ng-click="addfacebookapi()">Connect Facebook Account</button>
									...
									<div class="clearfix"> </div>   
                                          <div modal="showModal2" close="cancel()" class="modal-content">
							               <div class="modal-body"> <h3>Please Wait......</h3></div>
											...
											<div class="clearfix"> </div>   
											 <div class="iinttitile">
														CURRENTLY INTEGRATED ACCOUNT WITH REVENUE ADVISER
											</div>
											 <div class="clearfix"> </div>  
									<div class="well1 white"> 
											<div class="row  shopyf ng-scope" ng-repeat="x in stores " >
											  <div class="col-sm-1"><i class="fa fa-shopping-cart fa-2x"></i></div>
											  <div class="col-sm-3">{{x.store_name}}</div>
											   <!--<div class="col-sm-2">{{x.store_apikey}}</div>
											    <div class="col-sm-2">{{x.store_app_password}}</div>-->
												 <div class="col-sm-5">{{x.store_url}}</div>
												 <div class="col-sm-3">{{x.create_time}}</div>
												 <div class="clearfix"> </div>   

												 <div class="row topmar">
														
														<div class="col-sm-12">
														<button class="sfbtn fleft" ng-click="setupdata(x.store_name,x.store_apikey,x.store_app_password,x.id);">Setup my Revenue Reports</button>
														<button class="dlbtn fleft" style="color: #fff;
...
padding: 8px 10px;
font-size: 14px;
"ng-click="delete_store(x.id)">Delete</button>
									 ...
									 <div class="clearfix"> </div>   
											...
											<div class="iinttitile">
												CURRENTLY INTEGRATED FACEBOOK ACCOUNT WITH REVENUE ADVISER
											</div>   
                                           <div class="well1 white"> 
            
            
														<div class="row topmar  fbcolr ng-scope" ng-repeat="fb in fb_accounts " >
															 <div class="col-sm-2 fbcolr"><i class="fa fa-facebook-square fa-2x"></i></div>
															<!-- <div class="col-sm-2">{{fb.app_id}}</div>
															 <div class="col-sm-2">{{fb.app_secret}}</div>-->
															 <div class="col-sm-7">{{fb.ads_act}}</div>
															  <div class="col-sm-3">{{fb.created_at}}</div>
															  
															   <div class="row  topmar">
				
															<div class="col-sm-12 ">
															  <button class="fbbtn fleft " ng-click="fb_setupdata(fb.ads_act,fb.id);">Setup my Revenue Reports</button>
															  <button class="fbbtn fleft " ng-click="delete_fb(fb.id)" style="color: #fff;
																background-color: #dc363d;
																border: none;
																min-width: 70px;
																border-radius: 5px;
...
$scope.cancel_fb = function() {
 ...
 $scope.loginformsubmit = function ()
 {
  ...
  $scope.setupdata = function (storename,key,password,store_id)
           {
                //If DIV is hidden it will be visible and vice versa.
                //$scope.validationError = $scope.validationError ? false : true;
				//alert(storename);
				 ...
				 $http({
					...
					url : '<?php echo base_url();?>index.php/intregation/store_setup',
							...
							headers: {'Content-Type': 'application/x-www-form-urlencoded'},
							data :JSON.stringify({storename:storename, store_key:key,password:password,store_id:store_id})
							 }).success(function(data) {
								 //alert(data);
							     console.log(data);
								 $timeout(function () {
   ...
   // $scope.fb_setupdata = function (app_id,app_secret)
    $scope.fb_setupdata = function (ads_act,fb_account_id)
           {
                //If DIV is hidden it will be visible and vice versa.
                //$scope.validationError = $scope.validationError ? false : true;
				//alert(storename);
				 ...
				 $http({
					...
					url : '<?php echo base_url();?>index.php/intregation/fb_setup',
							...
							data :JSON.stringify({ads_act:ads_act,fb_account_id:fb_account_id})
							 }).success(function(data) {
								 //alert(data);
							     console.log(data);
								  $timeout(function () {
